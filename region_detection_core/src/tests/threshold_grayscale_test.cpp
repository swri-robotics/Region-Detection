...
static void updateImage( int, void* )
{
  ...
  if(contour_cfg.enable == 1)
  {
    ...
    for( int i = 0; i< contours.size(); i++ )
    {
     ...
     std::string summary = boost::str(boost::format("c[%i]: s: %i, area: %f, arc %f; (p0: %i, pf: %i); h: %i") %
...
int main( int argc, char** argv )
{
  ...
  createTrackbar( "Threshold Enable",
                  window_name, &threshold_enabled,
                  max_threshold_enabled, updateImage ); // Create a Trackbar to choose type of Threshold

  createTrackbar( trackbar_type,
                  window_name, &threshold_type,
                  max_type, updateImage ); // Create a Trackbar to choose type of Threshold

  createTrackbar( trackbar_value,
                  window_name, &threshold_value,
                  max_value, updateImage ); // Create a Trackbar to choose Threshold value

  //! [Canny trackbar]
  createTrackbar( "Canny Enable", window_name,
          &canny_cfg.enable,canny_cfg.MAX_ENABLE,
          updateImage );
  createTrackbar( "Canny Lower Threshold", window_name,
          &canny_cfg.lower_threshold, canny_cfg.MAX_LOWER_TH,
          updateImage );
  createTrackbar( "Canny Upper Threshold", window_name,
          &canny_cfg.upper_threshold, canny_cfg.MAX_UPPER_TH,
          updateImage );
  ...
  //! [Contour trackbar]
  createTrackbar( "Contour Enable", window_name,
          &contour_cfg.enable,contour_cfg.MAX_ENABLE,
          updateImage );
  createTrackbar( "Contour Method", window_name,
          &contour_cfg.method, contour_cfg.MAX_METHOD,
          updateImage );
  createTrackbar( "Contour Mode", window_name,
          &contour_cfg.mode, contour_cfg.MAX_MODE,
          updateImage );

  createTrackbar( "Save Image", window_name,
          &SAVE_IMAGE, 1,
          updateImage );
